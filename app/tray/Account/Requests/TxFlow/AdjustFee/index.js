...
function getMaxTotalFee(tx = { networkId: '' }) {
  const networkId = parseInt(tx.networkId)

  // for ETH-based chains, the max fee should be 2 ETH
  if ([1, 3, 4, 5, 6, 10, 42, 61, 62, 63, 69, 8453, 42161, 421611, 7777777].includes(networkId)) {
    return 2 * 1e18
  }

  // for Fantom, the max fee should be 250 FTM
  if ([250, 4002].includes(networkId)) {
    return 250 * 1e18
  }
...
const FeeOverlayInput = ({ initialValue, labelText, tabIndex, decimals, onReceiveValue, limiter }) => {
  ...
  return (
    <>
      ...
      <div id={labelId} className='gasRateLabel'>
        {labelText}
      </div>
    </>
  )
...
class TxFeeOverlay extends Component {
  ...
  render() {
    ...
    return (
      <div className='txAdjustFee vaultCardShow' ref={this.moduleRef}>
        {usesBaseFee(data) ? (
          <>
            <BaseFeeInput
