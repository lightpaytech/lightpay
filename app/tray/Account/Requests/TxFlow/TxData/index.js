    ...
    return (
      <div className='_flowMain' style={{ animationDelay: 0.1 * this.props.i + 's' }}>
        <div className='_flowDataInner'>
          <div className='_flowLabel'>Data</div>
          <div
            className='_flowDataValues'
            onClick={() => {
          ...
          >
            {req.data.data && req.data.data !== '0x' && req.data.data !== '0x0' ? (
              req.decodedData && req.decodedData.method ? (
                ...
                <div className='_flowDataVal'>{'Unknown data present'}</div>
              )
            ) : (
              <div className='_flowTag'>{'No Data Included'}</div>
            )}
            {req.data.data && req.data.data !== '0x' && req.data.data !== '0x0' ? (
              <div className='_flowTag _flowTagWarn'>{'Sending Data!'}</div>
            ) : null}
          </div>
        </div>
      </div>
