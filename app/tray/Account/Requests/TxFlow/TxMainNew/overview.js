...
import EnsOverview from '../../EnsActions'
...
const SimpleContractCallOverview = ({ method }) => {
  ...
  return <div className='_flowEnsLine'>{body}</div>
}
...
function renderRecognizedActions(req) {
  ...
  return !actions.length ? (
    <div className='_flowEnsLine'>Calling Contract</div>
  ) : (
      ...
      <Cluster>
        ...
        {replacementStatus.replacement &&
          (replacementStatus.possible ? (
            <ClusterRow>
              <ClusterValue>
                <div className='_flowTag _txMainTagGood'>valid replacement</div>
              </ClusterValue>
            </ClusterRow>
          ) : (
            <ClusterRow>
              <ClusterValue>
                <div className='_flowTag _txMainTagBad'>
        ...
        {isNonZeroHex(calldata) && (
          <ClusterRow>
            <ClusterValue>
              <div className='_flowTag _flowTagWarn'>{'Transaction includes data'}</div>
            </ClusterValue>
          </ClusterRow>
        )}
      </Cluster>
