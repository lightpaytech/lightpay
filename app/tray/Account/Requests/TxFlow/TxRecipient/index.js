      ...
      <ClusterBox title={title} animationSlot={this.props.i}>
        <Cluster>
          ...
          {req.decodedData && req.decodedData.method ? (
            <ClusterRow>
              <ClusterValue>
                <span className={'dataGroupTag'} style={{ color: 'var(--good)', fontSize: '16px' }}>
                  {(() => {
            ...
            <ClusterRow>
              <ClusterValue>
                <div className='dataGroupTag'>{'unknown action via unknown contract'}</div>
              </ClusterValue>
            </ClusterRow>
          ) : null}
          {req.decodedData && req.decodedData.source && (
            <ClusterRow>
              <ClusterValue>
                <div className='dataGroupTag'>{'abi source: ' + req.decodedData.source}</div>
              </ClusterValue>
            </ClusterRow>
          )}
        </Cluster>
