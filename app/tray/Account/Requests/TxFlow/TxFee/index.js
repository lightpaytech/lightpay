...
const GasDisplay = ({ maxFeePerGas }) => {
  ...
  return (
    <div data-testid='gas-display' className='_flowGasGwei'>
      <span className='_flowGasGweiValue'>{displayValue}</span>
      <span className='_flowGasGweiLabel'>{displayLabel}</span>
    </div>
  )
}
...
const USDEstimateDisplay = ({ minFee, maxFee, nativeCurrency }) => {
  ...
  return (
    <div data-testid='usd-estimate-display' className='dataGroupTag'>
      <div className={`_flowGasDefault${displayMaxFeeWarning ? ' _flowGasWarn' : ''}`}>
...
class TxFee extends React.Component {
  ...
  getOptimismFee = (l2Price, l2Limit, networkData) => {
    const l1DataFee = BigNumber(networkData?.l1Fees).toNumber()
  ...
  render() {
    ...
    const chain = {
      type: 'ethereum',
      id: parseInt(req.data.networkId, 16)
    }
    ...
    const maxFeeSourceValue = chainUsesOptimismFees(chain.id)
      ? this.getOptimismFee(maxFeePerGas, maxGas, req.networkData?.optimism)
    ...
    const minFeeSourceValue = chainUsesOptimismFees(chain.id)
      ? this.getOptimismFee(minFeePerGas, minGas, req.networkData?.optimism)
      ...
      <ClusterBox title='fee' animationSlot={this.props.i}>
        <Cluster>
          ...
          {req.feesUpdatedByUser ? (
            <ClusterRow>
              <ClusterValue>
                <div className='dataGroupTag' style={{ color: 'var(--good)' }}>
                  {`Gas values set by user`}
                </div>
              </ClusterValue>
            </ClusterRow>
          ) : req.data.gasFeesSource !== GasFeesSource.LightPay ? (
            <ClusterRow>
              <ClusterValue>
                <div className='dataGroupTag' style={{ color: 'var(--bad)' }}>
