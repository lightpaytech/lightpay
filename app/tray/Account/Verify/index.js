...
class Verify extends React.Component {
  ...
  getText(deviceType) {
    const isHwSigner = deviceType === 'seed' || deviceType === 'ring'
  ...
  render() {
    const deviceType = this.store('main.accounts', this.props.id, 'lastSignerType')
    const buttonClasses = ['sectionBtn']
    ...
    return (
      <div ref={this.moduleRef} className='assetsBlock'>
        <div className='sectionTitle'>{'Verify Address'}</div>
        <div className='sectionContent'>
          <div className='signerVerifyText'>Verify that the address displayed in LightPay is correct</div>
          {this.state.verifyAddressResponse ? (
            <div
              className={
                this.state.verifyAddressSuccess
                  ? 'deviceVerifyResp signerVerifyResponseSuccess vaultCardShow'
                  : 'deviceVerifyResp'
              }
            >
              {this.state.verifyAddressResponse}
