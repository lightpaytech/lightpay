...
class Balance extends React.Component {
  render() {
    const { symbol, balance, i, scanning, networkId } = this.props
    ...
    const chain = this.store('main.networks.ethereum', networkId)
    const { name: networkName = '', isTestnet = false } = chain
    const chainColor = this.store('main.networksMeta.ethereum', networkId, 'primaryColor')

    const ethMatch = logoURI?.includes('/coins/images/279/large/ethereum.png')

    return (
      <div
        className={i === 0 ? 'assetBalance signerBalanceBase' : 'assetBalance'}
        key={symbol}
