...
import Menu from './NavPanel'
import Badge from './StatusBar'

import Backdrop from './Veil'
  ...
  indicator(connection) {
    ...
    if (status.indexOf('connected') > -1) {
      if (this.store('selected.current')) {
        return <div className='walletStatusDot walletStatusGood' />
      } else {
        return <div className='walletStatusDot walletStatusPending' />
      }
    } else {
      return <div className='walletStatusDot walletStatusBad' />
    }
  }
