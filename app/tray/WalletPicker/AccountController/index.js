...
class Account extends React.Component {
  ...
  renderType() {
    ...
    return (
      <div
        className={!isHotSigner ? 'deviceType' : 'deviceType signerTypeHot'}
        onMouseDown={() => {
  ...
  renderMenu() {
    let menuClass = 'deviceMenu'
    menuClass += this.store('selected.view') === 'settings' ? ' deviceMenuSettings' : ' deviceMenuDefault'
    if ((this.store('selected.current') === this.props.id) & this.store('selected.open'))
      menuClass += ' deviceMenuOpen'
    return (
      <div className={menuClass}>
        <div
          className='deviceMenuItem deviceMenuItemLeft'
          onMouseDown={() => this.store.setSignerView('default')}
        >
          <div className='deviceMenuItemIcon'>
            {svg.octicon('pulse', { height: 23 })}
            <div className='iconUnderline' />
          </div>
        </div>
        <div
          className='deviceMenuItem deviceMenuItemRight'
  ...
  renderStatus() {
    ...
    return this.props.status !== 'ok' ? (
      <div className='signerStatusNotOk'>
        <div className='signerStatusNotOkInner'>
          <div className='signerStatusNotOkDot' />
          <div className='signerStatusNotOkText'>{status || 'connecting'}</div>
        </div>
        <div className='signerStatusNotOkLoader'>
          <div className='signerStatusNotOkLoaderBar' />
        </div>
      </div>
    ) : (
      <>
