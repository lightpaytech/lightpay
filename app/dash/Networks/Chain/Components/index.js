...
export const SubmitChainButton = ({ text, enabled, textColor, onClick }) => {
  return (
    <div
      role='button'
      className={enabled ? 'addAssetSubmit addAssetSubmitReady' : 'addAssetSubmit'}
  ...
  return (
    <div className='signerTop'>
      <div className='signerDetails'>
        ...
        {/* <div className='deviceType' style={this.props.inSetup ? {top: '21px'} : {top: '24px'}}>{this.props.model}</div> */}
        <div role='networkName' className='signerName'>
          {name}
        </div>
      </div>
      <div className='deviceMenuItems'>
        {showExpand && (
          <div
        ...
        {showToggle && (
          <div
            className={on ? 'devicePermToggle signerPermissionToggleOn' : 'devicePermToggle'}
...
export const EditChainColor = ({ currentColor, onChange }) => {
  return (
    <div className='networkRow'>
      <div className='networkInputLabel'>Chain Color</div>
      <div className='networkColorSwatches'>
        {accents.map((color) => (
          <div
            key={color}
            className={
...
export const EditChainField = ({ currentValue, defaultValue, label, onChange }) => {
  ...
  return (
    <div className='networkRow'>
      <label htmlFor={id} className='networkInputLabel'>
        {label}
      </label>
      <input
        id={id}
        className={!currentValue ? 'networkInput networkInputDim' : 'networkInput'}
...
export const EditChainId = ({ networkId, onChange }) => (
  <EditChainField
    currentValue={networkId}
    onChange={onChange}
    label={'Chain ID'}
...
export const EditTestnet = ({ testnet, onChange }) => {
  return (
    <div className='networkRowTestnet'>
      <label>Testnet</label>
      <div
        role='chainTestnet'
        aria-checked={testnet}
        className={testnet ? 'devicePermToggle signerPermissionToggleOn' : 'devicePermToggle'}
