...
class AddToken extends React.Component {
  constructor(props, context) {
    ...
    const networkId = parseInt(this.token.networkId)
    ...
    this.state = {
      ...
      networkId: (Number.isInteger(networkId) && networkId) || this.chainDefault,
  ...
  render() {
    const newTokenReady =
      ...
      Number.isInteger(this.state.networkId) &&
      this.state.address &&
    ...
    return (
      <div className='alertWrap' onMouseDown={(e) => e.stopPropagation()}>
        <div className='alertBoxSlide'>
          <div className='addAssetTitle'>Add New Token</div>
          <div className='addAsset'>
            ...
            <div className='tokenRow'>
              ...
              <div className='tokenChainId'>
                ...
                <input
                  className={
                    this.state.networkId === this.chainDefault ? 'tokenInput tokenInputDim' : 'tokenInput'
                  }
                  value={this.state.networkId}
                  spellCheck='false'
                  onChange={(e) => {
                    if (!e.target.value) return this.setState({ networkId: '' })

                    const networkId = parseInt(e.target.value)
                    if (!Number.isInteger(networkId)) return e.preventDefault()

                    this.setState({ networkId })
                  }}
                  onFocus={(e) => {
                    if (e.target.value === this.chainDefault) this.setState({ networkId: '' })
                  }}
                  onBlur={(e) => {
                    if (e.target.value === '') this.setState({ networkId: this.chainDefault })
                  }}
                />
              </div>
            </div>
            ...
            <div className='tokenRow'>
              {newTokenReady ? (
                <div
                  className='addAssetSubmit addAssetSubmitReady'
                  onMouseDown={() => {
                    const { name, symbol, networkId, address, decimals, logoURI } = this.state
                    const token = { name, symbol, networkId, address, decimals, logoURI }
                    link.send('tray:addAsset', token)
                    setTimeout(() => {
                      this.store.notify()
                    }, 400)
                  }}
                ...
                <div className='addAssetSubmit'>Fill in Token Details</div>
              )}
            </div>
          </div>
        </div>
