...
class Signer extends React.Component {
  ...
  renderPreview() {
    ...
    return (
      <div className={signerClass + ' vaultCardShow'} style={{ zIndex }}>
  ...
  renderExpanded() {
    ...
    // UI changes for this status only apply to hot signers
    const isLocked = !hwSigner && status === 'locked'
    const permissionId = tag || tag === '' ? 'LightPay' + (tag ? `-${tag}` : '') : undefined

    const zIndex = 1000 - index

    return (
      <div className={'expandedSigner vaultCardShow'} style={{ zIndex }}>
