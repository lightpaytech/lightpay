// LightPay Hardhat configuration
/* globals task */
...
taskWithDefaultParams('send-tx', 'send a test transaction')
  ...
  .setAction(
    async ({ amount, chain = 4, to = '0xf2C1E45B6611bC4378c3502789957A57e0390B79', provider = 'lightpay' }) => {
      ...
      const sendRequest = async () => {
        ...
        const eth = ethProvider(provider === 'hardhat' ? 'http://127.0.0.1:8545' : provider, {
          origin: 'lightpay-hardhat-worker'
        })
  ...
  commit: () => {
    return {
      ...
      data: registrarControllerContract.encodeFunctionData('commit', [
        utils.formatBytes32String('testing-lightpay')
      ])
    }
  },
