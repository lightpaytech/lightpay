...
describe('#okProtocol', () => {
  ...
  it('does not allow a connection to LightPay using localhost', () => {
    expect(okProtocol('http://localhost:1248')).toBe(false)
  })

  it('does not allow a connection to LightPay using an IP address', () => {
  ...
  it('does not allow an unknown preset', () => {
    expect(okProtocol('lightpay')).toBe(true)
  })
