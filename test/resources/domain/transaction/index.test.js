...
describe('#normalizeChainId', () => {
  it('does not modify a transaction with no chain id', () => {
    const tx = { to: '0xlightpay' }

    expect(normalizeChainId(tx)).toStrictEqual(tx)
  })

  it('normalizes a hex-prefixed chain id', () => {
    const tx = { to: '0xlightpay', chainId: '0xa' }

    expect(normalizeChainId(tx)).toStrictEqual({ to: '0xlightpay', chainId: '0xa' })
  })

  it('does not handle a hex chain id with no prefix', () => {
    const tx = { to: '0xlightpay', chainId: 'a' }
  ...
  it('normalizes a numeric chain id', () => {
    const tx = { to: '0xlightpay', chainId: 14 }

    expect(normalizeChainId(tx)).toStrictEqual({ to: '0xlightpay', chainId: '0xe' })
  })

  it('normalizes a numeric string chain id', () => {
    const tx = { to: '0xlightpay', chainId: '100' }

    expect(normalizeChainId(tx)).toStrictEqual({ to: '0xlightpay', chainId: '0x64' })
  })
  ...
  it('normalizes mainnet chain id from decimal string', () => {
    const tx = { to: SAMPLE_ADDRESS, chainId: '1' }

    expect(normalizeChainId(tx)).toStrictEqual({ to: SAMPLE_ADDRESS, chainId: MAINNET_CHAIN_ID })
  })

  it('normalizes polygon chain id from decimal', () => {
    const tx = { to: SAMPLE_ADDRESS, chainId: 137 }

    expect(normalizeChainId(tx)).toStrictEqual({ to: SAMPLE_ADDRESS, chainId: POLYGON_CHAIN_ID })
  })
})
