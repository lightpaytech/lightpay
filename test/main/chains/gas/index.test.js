...
// ─── Shared fee history fixtures ─────────────────────────────────────────────

const singleBlockFeeHistory = [
  { baseFee: 8, gasUsedRatio: 0, rewards: [0] },
  { baseFee: 182, rewards: [] }
]

const normalBlocksFeeHistory = [
  { baseFee: 8, gasUsedRatio: 0, rewards: [0] },
...
describe('#createGasCalculator', () => {
  describe('default gas calculator', () => {
    ...
    it('calculates the base fee for the next block', async () => {
      const { maxBaseFeePerGas } = await gasCalculator.calculateGas(singleBlockFeeHistory)
    ...
    it('calculates the priority fee for the next block based on normal blocks', async () => {
      ...
      const { maxPriorityFeePerGas } = await gasCalculator.calculateGas(normalBlocksFeeHistory)
    ...
    it('uses the priority fee from the latest block when no eligible blocks are available', async () => {
      const { maxBaseFeePerGas, maxPriorityFeePerGas } = await gasCalculator.calculateGas(singleBlockFeeHistory)
      expect(maxBaseFeePerGas).toBe('0xe7')
      expect(maxPriorityFeePerGas).toBe('0x0')
    })

    it('uses LightPay default config when no chainId is provided to createGasCalculator', async () => {
      // The default calculator should behave identically whether we call createGasCalculator()
      // or createGasCalculator(undefined) – both must process fee history without a chain override.
      const defaultCalc = createGasCalculator()
      const undefinedCalc = createGasCalculator(undefined)

      const resultDefault = await defaultCalc.calculateGas(normalBlocksFeeHistory)
      const resultUndefined = await undefinedCalc.calculateGas(normalBlocksFeeHistory)

      expect(resultDefault.maxPriorityFeePerGas).toBe(resultUndefined.maxPriorityFeePerGas)
      expect(resultDefault.maxBaseFeePerGas).toBe(resultUndefined.maxBaseFeePerGas)
    })
  })
  ...
  describe('polygon gas calculator', () => {
    ...
    it('does not change the priority fee if above 30 gwei', async () => {
      const polygonCalc = createGasCalculator(137)

      const { maxPriorityFeePerGas } = await polygonCalc.calculateGas([
