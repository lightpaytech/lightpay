...
beforeEach(() => {
  store.set('main.lattice', 'NBaJ8e', {
    deviceName: 'LightPay-testlattice',
...
describe('#remove', () => {
  ...
  it('closes a known Lattice signer', () => {
    adapter.knownSigners['M8jl93'] = { deviceName: 'existing-lightpay-lattice' }
  ...
  it('does not attempt to close an unknown Lattice signer', () => {
    adapter.knownSigners['G6s8sa'] = { deviceName: 'existing-lightpay-lattice' }
    adapter.remove(latticeSigner)
...
describe('signer device changes', () => {
  ...
  describe('detecting a new Lattice', () => {
    it('creates a new signer', (done) => {
      adapter.once('add', (lattice) => {
        try {
          ...
          expect(lattice.deviceName).toBe('LightPay-testlattice')
          done()
        } catch (e) {
          done(e)
        }
    ...
    it('does not create a new signer from one that is already known', () => {
      adapter.knownSigners['NBaJ8e'] = { deviceName: 'existing-lightpay-lattice' }
  ...
  describe('signer events', () => {
    ...
    it('handles update events', () => {
      ...
      expect(updateHandler).toHaveBeenCalledWith(
        expect.objectContaining({ deviceId: 'NBaJ8e', deviceName: 'LightPay-testlattice' })
      )
    })
