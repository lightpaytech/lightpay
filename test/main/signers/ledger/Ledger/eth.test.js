...
// Hex encoding of the message used when these APDU exchanges were recorded.
// The replay codes below must stay in sync with this value.
const RECORDED_SIGN_MESSAGE_HEX = '68656c6c6f2c204672616d6521' // "hello, Frame!" — original recording
const RECORDED_SIGN_MESSAGE_HEX_PREFIXED = `0x${RECORDED_SIGN_MESSAGE_HEX}`

// LightPay-branded message hex for new tests (not backed by a hardware replay)
const LIGHTPAY_SIGN_MESSAGE = '68656c6c6f2c204c6967687450617921' // "hello, LightPay!"

const LEGACY_TX_FIXTURE = {
...
describe('#signMessage', () => {
  describe('successful signing', () => {
    it('signs a message with a hex prefix', async () => {
      const replayData = `
        => e008000026058000002c8000003c8000000000000000000000020000000d${RECORDED_SIGN_MESSAGE_HEX}
      ...
      const ethApp = await createEthApp(replayData)
      const signature = await ethApp.signMessage("44'/60'/0'/0/2", RECORDED_SIGN_MESSAGE_HEX_PREFIXED)
    ...
    it('signs a message with no hex prefix', async () => {
      const replayData = `
        => e008000026058000002c800000018000000000000000000000060000000d${RECORDED_SIGN_MESSAGE_HEX}
      ...
      const ethApp = await createEthApp(replayData)
      const signature = await ethApp.signMessage("44'/1'/0'/0/6", RECORDED_SIGN_MESSAGE_HEX)
  ...
  describe('error cases', () => {
    it('fails to sign a message with an invalid BIP 32 path', async () => {
      const replayData = `
        => e008000012000000000d${RECORDED_SIGN_MESSAGE_HEX}
        <= 6a80
      `
      ...
      try {
        await ethApp.signMessage('badpath', RECORDED_SIGN_MESSAGE_HEX_PREFIXED)
    ...
    it('fails to sign a message when rejected by the user', async () => {
      const replayData = `
        => e008000022048000002c8000003c80000001000000040000000d${RECORDED_SIGN_MESSAGE_HEX}
        <= 6985
      `
      ...
      try {
        await ethApp.signMessage("44'/60'/1'/4", RECORDED_SIGN_MESSAGE_HEX_PREFIXED)
  ...
  describe('LightPay message constant', () => {
    it('LIGHTPAY_SIGN_MESSAGE is the hex encoding of "hello, LightPay!"', () => {
      const decoded = Buffer.from(LIGHTPAY_SIGN_MESSAGE, 'hex').toString('utf8')
      expect(decoded).toBe('hello, LightPay!')
    })
  })
})
...
describe('#signTypedData', () => {
  describe('successful signing', () => {
    it('signs valid v4 typed data', async () => {
      ...
      const ethApp = await createEthApp(replayData)
      const signature = await ethApp.signTypedData("44'/60'/0'/0", TYPED_DATA_FIXTURE)
  ...
  describe('error cases', () => {
    it('fails to sign typed data with an invalid BIP 32 path', async () => {
      ...
      try {
        await ethApp.signTypedData('badpath', TYPED_DATA_FIXTURE)
    ...
    it('fails to sign typed data when rejected by the user', async () => {
      ...
      try {
        await ethApp.signTypedData("44'/60'/0'/0", TYPED_DATA_FIXTURE)
...
describe('#signTransaction', () => {
  describe('legacy (pre-EIP-1559) transactions', () => {
    it('signs a pre-EIP-1193 transaction', async () => {
      ...
      `
      const ethApp = await createEthApp(replayData)
      const signature = await ethApp.signTransaction("44'/60'/0'/0", LEGACY_TX_FIXTURE)
  ...
  describe('EIP-1559 transactions', () => {
    it('signs an EIP-1559 transaction', async () => {
      ...
      const ethApp = await createEthApp(replayData)
      const signature = await ethApp.signTransaction("44'/60'/0'/0", EIP1559_TX_FIXTURE)
  ...
  describe('error cases', () => {
    it('fails to sign a transaction with an invalid BIP 32 path', async () => {
      ...
      try {
        await ethApp.signTransaction('badpath', EIP1559_TX_FIXTURE)
    ...
    it('fails to sign a transaction when rejected by the user', async () => {
      ...
      try {
        await ethApp.signTransaction("44'/60'/0'/0", EIP1559_TX_FIXTURE)
