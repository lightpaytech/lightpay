...
describe('#signTransaction', () => {
  describe('known chains', () => {
    CHAINS.forEach((chain) => {
      it(`signs a transaction on ${chain.name}`, (done) => {
        const tx = { ...BASE_RAW_TX, chainId: chain.chainId.toString(16) }
  ...
  describe('edge cases', () => {
    ...
    it('rejects a transaction when key is null', (done) => {
      const tx = { ...BASE_RAW_TX, chainId: (1).toString(16) }

      worker.signTransaction(null, tx, (err) => {
        try {
