...
import { fetchEtherscanContract, buildEtherscanUrl } from '../../../../main/contracts/sources/etherscan'
...
describe('#buildEtherscanUrl', () => {
  it('builds the correct Etherscan API URL for mainnet', () => {
    ...
    expect(url).toBe(
      `https://api.etherscan.io/api?module=contract&action=getsourcecode&address=${sampleContractAddress}&apikey=3SYU5MW5QK8RPCJV1XVICHWKT774993S24`
    )
  })

  it('builds the correct Etherscan API URL for Polygon', () => {
    const url = buildEtherscanUrl(
      sampleContractAddress,
      'api.polygonscan.com',
    ...
    expect(url).toContain('api.polygonscan.com')
    expect(url).toContain(sampleContractAddress)
...
describe('#fetchEtherscanContract', () => {
  const getPath = (apiKey) =>
    `/api?module=contract&action=getsourcecode&address=${sampleContractAddress}&apikey=${apiKey}`
  ...
  describe('supported chains', () => {
    supportedChains.forEach((chain) => {
      ...
      it(`retrieves a contract from ${name}`, async () => {
        ...
        return expect(fetchEtherscanContract(sampleContractAddress, chain.chainId)).resolves.toStrictEqual({
          abi: JSON.stringify(sampleAbi),
          name: `mock ${name} abi`,
          source: name
        })
  ...
  describe('error and edge cases', () => {
    it('does not retrieve a contract from etherscan when the request fails', async () => {
      ...
      return expect(fetchEtherscanContract(sampleContractAddress, 1)).resolves.toBeUndefined()
    }, 200)
    ...
    it('does not retrieve a contract from an unsupported chain', async () => {
      return expect(fetchEtherscanContract(sampleContractAddress, 4)).resolves.toBeUndefined()
    }, 200)
    ...
    it('does not retrieve a contract when the request times out', async () => {
      ...
      const contract = expect(fetchEtherscanContract(sampleContractAddress, 1)).resolves.toBeUndefined()
