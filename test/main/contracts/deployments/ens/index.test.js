...
describe('registrar', () => {
  ...
  describe('transfers', () => {
    ...
    supportedFunctions.forEach((fn) => {
      ...
      it(`resolves the ENS name for a ${fn} call from the user's asset collection`, () => {
        ...
        expect(action).toStrictEqual({
          id: 'ens:transfer',
          data: { name: 'lightpay.eth', from, to, tokenId }
        })
      })
    })
  })
  ...
  describe('approvals', () => {
    ...
    it(`resolves the ENS name for an approve call from the user's asset collection`, () => {
      ...
      expect(action).toStrictEqual({
        id: 'ens:approve',
        data: { name: 'lightpay.eth', operator: to, tokenId }
      })
    })
  })
})
  ...
  describe('registrations', () => {
    ...
    supportedFunctions.forEach((fn) => {
      it(`recognizes a call to ${fn} in order to register an ENS name`, () => {
        ...
        const functionParams = ['lightpay.eth', to, duration, utils.formatBytes32String('asecretphrase')].concat(
        ...
        expect(action).toStrictEqual({
          id: 'ens:register',
          data: { name: 'lightpay.eth', address: to, duration }
        })
      })
    })
    ...
    it('adds a .eth extension to a name to be registered', () => {
      ...
      expect(action.data.name).toBe('lightpay.eth')
    })
  })

  it('recognizes a call to renew an ENS name', () => {
    const duration = 60 * 60 * 24 * 30 * 18 // 18 months, in seconds
    const calldata = registrarControllerInterface.encodeFunctionData('renew', ['lightpay.eth', duration])
    const action = registrarController.decode(calldata)

    expect(action).toStrictEqual({
      id: 'ens:renew',
      data: { name: 'lightpay.eth', duration }
    })
  })
})
