  ...
  return {
    ...
    apply: (state) => {
      return mockLatestVersion === 2
        ? { ...state, main: { ...state.main, _version: 2, instanceId: 'test-brand-new-lightpay' } }
        : { ...state }
    }
  }
})
      ...
      return {
        __: {
          1: {
            main: {
              _version: 1,
              instanceId: 'test-lightpay'
            }
          },
          2: {
            main: {
              _version: 2,
              instanceId: 'test-brand-new-lightpay'
            }
          }
        }
      }
...
it('maintains backwards compatible access to the current version of state', async () => {
  ...
  expect(state().main.instanceId).toBe('test-lightpay')
})
...
it('loads values from the current version of the state', async () => {
  ...
  expect(state().main.instanceId).toBe('test-brand-new-lightpay')
})
...
it('preserves an older version of the state after creating a newer state entry', async () => {
  ...
  expect(writtenState.main.__['1'].main.instanceId).toBe('test-lightpay')
  expect(writtenState.main.__['2'].main.instanceId).toBe('test-brand-new-lightpay')
}, 500)
