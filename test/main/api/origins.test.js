...
describe('#updateOrigin', () => {
  describe('handling origins', () => {
    it('adds a new origin to the store', () => {
      updateOrigin({}, 'lightpay.test')

      expect(store.initOrigin).toHaveBeenCalledWith(uuidv5('lightpay.test', uuidv5.DNS), {
        name: 'lightpay.test',
        chain: {
          type: 'ethereum',
          id: 1
    ...
    it('does not overwrite an existing origin', () => {
      store.set('main.origins', uuidv5('lightpay.test', uuidv5.DNS), { chain: { id: 1 } })
    ...
    it('does not initialize a new origin on a connection message', () => {
      updateOrigin({}, 'lightpay.test', true)

      expect(store.initOrigin).not.toHaveBeenCalled()
    })
    it('sets the payload chain id to mainnet for connection messages with no known origin', () => {
      const originalPayload = {}
      const { payload, chainId } = updateOrigin(originalPayload, 'lightpay.test', true)
    ...
    it('sets the chain id to mainnet for a new origin', () => {
      const { chainId } = updateOrigin({}, 'lightpay.test')
    ...
    it('sets the chain id for an existing origin', () => {
      store.set('main.origins', uuidv5('lightpay.test', uuidv5.DNS), { chain: { id: 137 } })
    ...
    it('does not override the chain id in the payload with one from a configured origin', () => {
      ...
      const { chainId } = updateOrigin({ chainId: '0x1' }, 'lightpay.test')
  ...
  describe('parsing', () => {
    ...
    it('does not change an origin with no prepended protocol', () => {
      const origin = parseOrigin('send.lightpay.tech')

      expect(origin).toBe('send.lightpay.tech')
    })

    it('does not change a plain string origin', () => {
      const origin = parseOrigin('lightpay-extension')

      expect(origin).toBe('lightpay-extension')
    })
...
describe('#parseLightPayExtension', () => {
  it('correctly identifies the Chrome extension', () => {
    ...
    expect(parseLightPayExtension(req)).toStrictEqual({
      browser: 'chrome',
  ...
  it('correctly identifies the Safari extension', async () => {
    return withEnvironment({ NODE_ENV: 'development' }, async () => {
      ...
      const { parseLightPayExtension } = await import('../../../main/api/origins')
...
describe('#isKnownExtension', () => {
  ...
  it('always knows the single Safari extension', async () => {
    const extension = { browser: 'safari', id: 'test-lightpay' }
    return expect(isKnownExtension(extension)).resolves.toBe(true)
  })
...
describe('#isTrusted', () => {
  const lightpayTestOriginId = 'bf93061b-3575-40c5-b526-4932b02e1f3f'

  beforeEach(() => {
    store.set('main.origins', lightpayTestOriginId, { name: 'test.lightpay.eth' })
    store.set('main.permissions', {})
  })
  ...
  describe('extension requests', () => {
    ...
    const trustedOrigins = ['lightpay-extension', 'lightpay-internal']
  ...
  it('does not trust a request if no account is selected', async () => {
    const payload = { _origin: lightpayTestOriginId }
  ...
  it('sends a request to grant permission to the user', async () => {
    ...
    accounts.addRequest.mockImplementationOnce((request, cb) => {
      expect(request).toStrictEqual({
        type: 'access',
        handlerId: lightpayTestOriginId,
        origin: lightpayTestOriginId,
        account: address,
        payload: {
          method: 'eth_accounts'
        }
  ...
  it('sends a response to all permission requests once the user trusts the origin', async () => {
    const address = '0xDAFEA492D9c6733ae3d56b7Ed1ADB60692c98Bc5'
    const payload1 = { method: 'wallet_getEthereumAccounts', _origin: lightpayTestOriginId }
    const payload2 = { method: 'eth_accounts', _origin: lightpayTestOriginId }
