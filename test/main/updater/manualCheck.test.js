...
const githubReleasesResponse = [
  {
    html_url: 'https://lightpay.tech/the-next-great-release',
    prerelease: false,
...
it('identifies that a newer version is available', async () => {
  const response = [
    {
      html_url: 'https://lightpay.tech/cutting-edge-lightpay-release',
      prerelease: true,
  ...
  expect(res.location).toBe('https://lightpay.tech/cutting-edge-lightpay-release')
})
...
function mockApiResponse(status, body, headers = { 'content-type': 'application/json' }) {
  nock('https://api.github.com').get('/repos/lightpay-labs/lightpay/releases').reply(status, body, headers)
}
