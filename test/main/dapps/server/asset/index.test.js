...
import { getAssetCacheKey } from '../../../../../main/dapps/server/asset'
...
beforeEach(() => {
  ...
  fs.createReadStream.mockReturnValue(Readable.from('hello, LightPay!'))
})
...
it('returns a stream with the asset contents', async () => {
  ...
  return expect(res.getResponseData()).resolves.toBe('hello, LightPay!')
})
...
describe('#getAssetCacheKey', () => {
  it('returns a cache key combining dappId and assetPath', () => {
    expect(getAssetCacheKey('0xabc', '/index.html')).toBe('0xabc::/index.html')
  })

  it('returns a distinct key for different dappIds with the same asset path', () => {
    const key1 = getAssetCacheKey('0xabc', '/app.js')
    const key2 = getAssetCacheKey('0xdef', '/app.js')
    expect(key1).not.toBe(key2)
  })

  it('returns a distinct key for the same dappId with different asset paths', () => {
    const key1 = getAssetCacheKey('0xabc', '/index.html')
    const key2 = getAssetCacheKey('0xabc', '/app.js')
    expect(key1).not.toBe(key2)
  })
})
