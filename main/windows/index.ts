...
// LightPay tray window — portfolio-enabled wallet interface
...
export class Tray {
  ...
  public show() {
    ...
    windows.tray.setMinimumSize(width, height)
    windows.tray.setSize(width, height)
    windows.tray.setMaximumSize(width, height)
...
class Dash {
  ...
  public show() {
    ...
    setTimeout(() => {
      ...
      const height = isDev && !fullheight ? devHeight : area.height
      const width = Math.floor(area.width / 2)
      windows.dash.setMinimumSize(width, height)
      windows.dash.setSize(width, height)
      windows.dash.setMaximumSize(width, height)
      windows.dash.setPosition(area.x, area.y)
      windows.dash.show()
...
class Onboard {
  ...
  public show() {
    ...
    setTimeout(() => {
      ...
      windows.onboard.setPosition(x, y)
...
class Notify {
  ...
  public show() {
    ...
    setTimeout(() => {
      ...
      const x = Math.floor(area.x + (area.width - width) / 2)
      const y = Math.floor(area.y + (area.height - height) / 2)
      windows.notify.setPosition(x, y)
      // windows.onboard.setAlwaysOnTop(true)
      windows.notify.show()
...
ipcMain.on('*:contextmenu', (e, x, y, modifierKeyPressed = false) => {
  if (isDev && modifierKeyPressed) {
