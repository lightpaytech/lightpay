  ...
  return {
    getCurrencyBalances: async function (address: string, chains: number[]) {
      if (!isValidAddress(address)) {
        log.warn(`getCurrencyBalances called with invalid address: ${address}`)
        return []
      }

      const validChains = chains.filter(isValidChainId)

      if (validChains.length !== chains.length) {
        log.warn(`getCurrencyBalances: filtered out ${chains.length - validChains.length} invalid chain id(s)`)
      }

      const fetchChainBalance = getNativeCurrencyBalance.bind(null, address)

      return Promise.all(validChains.map(fetchChainBalance))
    },
    getTokenBalances: async function (owner: string, tokens: TokenDefinition[]) {
      if (!isValidAddress(owner)) {
        log.warn(`getTokenBalances called with invalid owner address: ${owner}`)
        return []
      }
