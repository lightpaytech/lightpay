...
interface BalanceUpdateStats {
  lastUpdate: number
  pendingCount: number
}

let _lastUpdateTimestamp = 0
let _pendingUpdateCount = 0

export function getBalanceUpdateStats(): BalanceUpdateStats {
  return {
    lastUpdate: _lastUpdateTimestamp,
    pendingCount: _pendingUpdateCount
  }
}
...
export default function (store: Store) {
  ...
  function handleUpdate(address: Address, updateFn: (address: Address) => void) {
    ...
    if (store('main.accounts', address)) {
      ...
      if (_pendingUpdateCount > 0) _pendingUpdateCount--
      updateFn(address)
    }
  }
