...
export function getControllerStatus(controller: BalancesWorkerController | null): ControllerStatus {
  return {
    running: controller !== null && controller.isRunning()
  }
}
...
export default class BalancesWorkerController extends EventEmitter {
  ...
  private spawnWorker(): ChildProcess {
    ...
    const worker = fork(path.resolve(__dirname, 'worker.js'), [], { execArgv: workerArgs })

    log.info('created balances worker, pid:', worker.pid)
