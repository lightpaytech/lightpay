...
export default class LatticeAdapter extends SignerAdapter {
  ...
  /**
   * Returns metadata about this adapter and the Lattice devices it currently
   * manages. Intended for diagnostics and health-check use.
   */
  getAdapterInfo(): {
    adapterType: string
    ...
    const devices = Object.values(this.knownSigners)
    return {
      adapterType: this.adapterType,
      trackedDevices: devices.length,
      deviceIds: Object.keys(this.knownSigners),
      pairedCount: devices.filter((d) => d.connection?.isPaired).length
    }
  }
  ...
  remove(lattice: Lattice) {
    log.info(`removing Lattice device ${lattice.deviceId} from known signers`)
  ...
  async reload(lattice: Lattice) {
    log.info(`reloading Lattice device ${lattice.deviceId}`)
    ...
    try {
      ...
      log.error(`could not reload Lattice device ${lattice.deviceId}`, e)
    }
  }
}
