...
export default class TrezorSignerAdapter extends SignerAdapter {
  ...
  /**
   * Returns metadata about this adapter and the Trezor devices it currently
  ...
  getAdapterInfo(): {
    ...
    return {
      ...
      trackedDevices: Object.keys(this.knownSigners).length,
  ...
  remove(trezor: Trezor) {
    if (trezor.id in this.knownSigners) {
      log.info(`removing Trezor device ${trezor.id} (model: ${trezor.model})`)
  ...
  private withSigner(device: TrezorDevice, fn: (signer: Trezor) => void) {
    const deviceId = Trezor.generateId(device.path)
    const signer = this.knownSigners[deviceId]?.signer

    if (!signer) {
      log.warn(`TrezorSignerAdapter: no known signer for device id ${deviceId}`)
      return
    }
