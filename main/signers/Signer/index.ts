...
export default class Signer extends EventEmitter {
  ...
  verifyAddress(index: number, current: string, display: boolean, cb: Callback<boolean>) {
    const err = new Error(`Signer [${this.type}/${this.id}]: verifyAddress is not implemented`)
    log.error(err)
    cb(err, undefined)
  }
  ...
  /**
   * Returns a lightweight summary containing only the signer's identity and
   * account count — useful for logging, diagnostics, and UI badges where the
   * full address list is not needed.
   */
  getSignerSummary(): { id: string; type: string; addresses: number } {
    return {
      id: this.id,
      type: this.type,
