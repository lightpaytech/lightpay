...
export default class LedgerEthereumApp {
  ...
  async deriveAddresses(derivation: Derivation) {
    log.info(`LedgerEthereumApp: deriving ${derivation} addresses`)
  ...
  async signTypedData(path: string, typedData: TypedData) {
    ...
    try {
      ...
      throw new DeviceError('LedgerEthereumApp: invalid or malformed typed data (EIP-712)', 99901)
    }
  ...
  async signTransaction(path: string, ledgerTx: TransactionData) {
    const signedTx = await sign(ledgerTx, (tx) => {
      // Legacy transactions aren't RLP encoded before they're returned from getMessageToSign
