...
export default class Ledger extends Signer {
  ...
  private getPath(index: number) {
    if (!this.derivation) {
      throw new Error(`Ledger [${this.id}]: attempted to get derivation path but derivation type is unknown`)
    }
  ...
  /**
   * Returns a compact description of the hardware device, suitable for logging
   * and diagnostics without exposing address data.
   */
  getDeviceInfo(): { id: string; model: string; devicePath: string; status: string; derivation: string | undefined } {
    return {
      id: this.id,
      model: this.model,
  ...
  private deriveLiveAddresses() {
    ...
    for (let i = 0; i < this.accountLimit; i++) {
      requests.push({
        ...
        execute: async () => {
          try {
            if (!this.eth) throw new Error(`Ledger [${this.id}]: attempted to derive Live addresses but Eth app is not connected`)
  ...
  private deriveHardwareAddresses() {
    ...
    this.enqueueRequests({
      ...
      execute: async () => {
        try {
          if (!this.eth)
            throw new Error(`Ledger [${this.id}]: attempted to derive hardware addresses but Eth app is not connected`)
          if (!this.derivation)
            throw new Error(`Ledger [${this.id}]: attempted to derive hardware addresses with unknown derivation`)
