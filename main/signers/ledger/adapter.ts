...
export default class LedgerSignerAdapter extends SignerAdapter {
  ...
  /**
   * Returns metadata about the adapter and currently tracked Ledger devices.
   * Useful for diagnostics, telemetry, and health-check endpoints.
   */
  getAdapterInfo(): {
    adapterType: string
    ...
    return {
      ...
      connectedDevices: Object.keys(this.knownSigners).length,
      pendingDisconnections: this.disconnections.length,
      devicePaths: Object.keys(this.knownSigners)
    }
  }
  ...
  private async handleAttachedDevice(device: ConnectedDevice) {
    log.info(`Ledger device attached — product: "${device.product}", path: ${device.path}`)
