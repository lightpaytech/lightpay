...
/**
 * Asserts that a required signer field is present.
 * @param {*} value - Value to check.
 * @param {string} fieldName - Human-readable name for error messages.
 * @param {string} signerId - Signer identifier for context.
 */
function assertSignerField(value, fieldName, signerId) {
  if (value === undefined || value === null) {
    throw new Error(`HotSigner [${signerId}]: required field "${fieldName}" is missing`)
  }
}
...
class HotSigner extends Signer {
  ...
  /**
   * Verifies that the address at the given index matches the expected address.
   * Locks the signer on mismatch or error to prevent unsafe use.
   * @param {number} index - Address index to verify.
   * @param {string} address - Expected Ethereum address.
   * @param {boolean} display - Whether to display the address on the device.
   * @param {function(Error|null, boolean|undefined): void} [cb] - Result callback.
   */
  verifyAddress(index, address, display, cb = () => {}) {
    ...
    this._callWorker(payload, (err, verified) => {
      if (err || !verified) {
        ...
        this.lock(() => {
          log.error(`HotSigner [${this.id}] verifyAddress failed: ${err.message}`)
          log.error(err)
        })
        cb(err)
      } else {
        log.info(`HotSigner [${this.id}] verify address matched`)
        cb(null, verified)
      }
    })
  }
  ...
  /**
   * Sends an RPC payload to the worker subprocess and invokes cb with the result.
   * Retries up to once per 100 ms if the worker token is not yet available.
   * @param {{ method: string, params?: object }} payload - RPC method and parameters.
   * @param {function(Error|null, *): void} cb - Node-style callback (err, result).
   */
  _callWorker(payload, cb) {
    if (!this._worker) throw Error(`HotSigner [${this.id}]: worker process is not running`)
