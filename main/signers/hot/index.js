...
/**
 * Validates the common password requirements for hot signer creation.
 * @param {string} password - The password to validate.
 * @returns {{ valid: boolean, error: string | null }} Validation result.
 */
function validateHotSignerPassword(password) {
  if (!password) return { valid: false, error: 'Password required to create hot signer' }
  if (password.length < 12) return { valid: false, error: 'Hot account password is too short' }
  if (zxcvbn(password).score < 3) return { valid: false, error: 'Hot account password is too weak' }
...
module.exports = {
  ...
  createFromSeed: (signers, seed, password, cb) => {
    ...
    const { valid, error } = validateHotSignerPassword(password)
    if (!valid) return cb(new Error(error))
    ...
    const scan = async () => {
      ...
      fs.readdirSync(SIGNERS_PATH).forEach((file) => {
        try {
          ...
          log.error(`HotSigner scan: corrupt or unreadable signer file "${file}"`, e)
        }
      })
