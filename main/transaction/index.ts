...
function populate(rawTx: TransactionData, chainConfig: Common, gas: Gas): TransactionData {
  ...
  if (!chainConfig.isActivatedEIP(1559) || !gas.price.fees) {
    ...
    const useLightPayGasPrice = !rawTx.gasPrice || isNaN(parseInt(rawTx.gasPrice, 16))
    if (useLightPayGasPrice) {
      // no valid dapp-supplied value for gasPrice so we use the LightPay-supplied value
      const gasPrice = BigNumber(gas.price.levels.fast as string).toString(16)
      txData.gasPrice = addHexPrefix(gasPrice)
      txData.gasFeesSource = GasFeesSource.LightPay
    }
  ...
  const useLightPayMaxFeePerGas = !rawTx.maxFeePerGas || isNaN(parseInt(rawTx.maxFeePerGas, 16))
  const useLightPayMaxPriorityFeePerGas =
    !rawTx.maxPriorityFeePerGas || isNaN(parseInt(rawTx.maxPriorityFeePerGas, 16))

  if (!useLightPayMaxFeePerGas && !useLightPayMaxPriorityFeePerGas) {
    // return tx unaltered when we are using no LightPay-supplied values
    return txData
  }
  ...
  const maxPriorityFee =
    useLightPayMaxPriorityFeePerGas && gas.price.fees.maxPriorityFeePerGas
  ...
  txData.maxFeePerGas =
    useLightPayMaxFeePerGas && gas.price.fees.maxBaseFeePerGas
      ? calculateMaxFeePerGas(gas.price.fees.maxBaseFeePerGas, maxPriorityFee)
      : txData.maxFeePerGas

  // if no valid dapp-supplied value for maxPriorityFeePerGas we use the LightPay-supplied value
  txData.maxPriorityFeePerGas = useLightPayMaxPriorityFeePerGas
