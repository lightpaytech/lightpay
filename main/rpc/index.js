...
// Named constants
const IPC_CHANNEL = 'main:rpc'
const LATTICE_BASE_URL = 'https://signing.gridpl.us'
const LATTICE_ENDPOINT_MODE = 'default'
const LATTICE_NAME_MAX_LENGTH = 14
const LATTICE_DEFAULT_NAME = 'GridPlus'
const LATTICE_TAG_LENGTH = 6
const LATTICE_PRIV_KEY_BYTES = 32
const TX_SENT_DELAY_MS = 1800
const ENS_RESOLVE_TIMEOUT_MS = 8000
const VALID_KEYSTORE_VERSIONS = [1, 3]
const ETH_DOMAIN_SUFFIX = '.eth'
const XYZ_DOMAIN_SUFFIX = '.xyz'

// Method category classification
const RPC_METHOD_CATEGORIES = {
  account: ['getAccounts', 'getCoinbase', 'setSigner', 'unsetSigner', 'createFromAddress', 'createAccount', 'removeAccount', 'verifyAddress'],
  signer: ['trezorPin', 'trezorPhrase', 'trezorEnterPhrase', 'latticePair', 'createLattice', 'unlockSigner', 'lockSigner', 'remove', 'createFromPhrase', 'createFromKeystore', 'createFromPrivateKey', 'addPrivateKey', 'removePrivateKey', 'addKeystore', 'locateKeystore'],
  transaction: ['approveRequest', 'declineRequest', 'updateRequest', 'confirmRequestApproval', 'setBaseFee', 'setPriorityFee', 'setGasPrice', 'setGasLimit', 'removeFeeUpdateNotice', 'signerCompatibility', 'signTransaction', 'signMessage'],
  provider: ['providerSend', 'connectionStatus', 'launchStatus'],
  dapp: ['addDapp', 'removeDapp', 'moveDapp', 'launchDapp', 'openDapp', 'flowCommand'],
  state: ['getState', 'getFrameId', 'respondToExtensionRequest'],
  ens: ['resolveEnsName'],
...
function getRpcMethodCategory(method) {
  for (const [category, methods] of Object.entries(RPC_METHOD_CATEGORIES)) {
    if (methods.includes(method)) return category
  }
  return 'unknown'
}

function incrementCallCount(method) {
  rpcCallCounts[method] = (rpcCallCounts[method] || 0) + 1
}
...
const rpc = {
  ...
  createLattice: (deviceId, deviceName, cb) => {
    ...
    store.updateLattice(deviceId, {
      deviceId,
      baseUrl: LATTICE_BASE_URL,
      endpointMode: LATTICE_ENDPOINT_MODE,
      paired: true,
      deviceName: (deviceName || LATTICE_DEFAULT_NAME).substring(0, LATTICE_NAME_MAX_LENGTH),
      tag: randomLetters(LATTICE_TAG_LENGTH),
      privKey: randomBytes(LATTICE_PRIV_KEY_BYTES).toString('hex')
    })
  ...
  approveRequest(req) {
    ...
    if (req.type === 'transaction') {
      provider.approveTransactionRequest(req, (err, res) => {
        ...
        setTimeout(() => accounts.setTxSent(req.handlerId, res), TX_SENT_DELAY_MS)
      })
  ...
  async resolveEnsName(name, cb) {
    ...
    try {
      ...
      } = await nebula.ens.resolve(name, { timeout: ENS_RESOLVE_TIMEOUT_MS })
      cb(null, ethAddress)
    } catch (err) {
  ...
  addDapp(domain, options, cb) {
    if (!(domain.endsWith(ETH_DOMAIN_SUFFIX) || domain.endsWith(XYZ_DOMAIN_SUFFIX))) domain += ETH_DOMAIN_SUFFIX
  ...
  openDapp(domain, options, cb) {
    if (domain.endsWith(ETH_DOMAIN_SUFFIX)) {
...
ipcMain.on(IPC_CHANNEL, (event, id, method, ...args) => {
  id = unwrap(id)
