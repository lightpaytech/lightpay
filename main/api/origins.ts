...
export interface LightPayExtension {
  browser: Browser
  id: string
}

// allows the LightPay extension to request specific methods
const trustedInternalMethods = ['wallet_getEthereumChains']

const isTrustedOrigin = (origin: string) => origin === 'lightpay-extension' || origin === 'lightpay-internal'
...
async function requestExtensionPermission(extension: LightPayExtension) {
...
export function parseLightPayExtension(req: IncomingMessage): LightPayExtension | undefined {
  const origin = req.headers.origin || ''

  const query = queryString.parse((req.url || '').replace('/', ''))
  const hasExtensionIdentity = query.identity === 'lightpay-extension'
  ...
  if (origin === 'chrome-extension://ldcoohedfbjoobcadoglnnmmfbdlmmhf') {
    ...
    return { browser: 'safari', id: 'lightpay-dev' }
  }
}

export async function isKnownExtension(extension: LightPayExtension) {
...
export async function isTrusted(payload: RPCRequestPayload) {
  // Permission granted to unknown origins only persist until LightPay is closed, they are not permanent
