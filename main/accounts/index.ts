...
import LightPayAccount from './Account'
    ...
    .reduce(
      ({ l1Transactions, l2Transactions }, [id, req]) => {
        ...
        if (
          ...
          txRequest.data.gasFeesSource === GasFeesSource.LightPay &&
...
const lightpayOriginId = uuidv5('lightpay-internal', uuidv5.DNS)
...
export class Accounts extends EventEmitter {
  _current: string
  accounts: Record<string, LightPayAccount>
  ...
  constructor() {
    ...
    this.accounts = Object.entries(storeApi.getAccounts()).reduce((accounts, [id, account]) => {
      accounts[id] = new LightPayAccount(JSON.parse(JSON.stringify(account)), this)

      return accounts
    }, {} as Record<string, LightPayAccount>)
  ...
  private getTransactionRequest(account: LightPayAccount, id: string): TransactionRequest {
    const req = account.getRequest(id)
    if (!req) throw new Error(`Transaction request ${id} not found`)
    return req as TransactionRequest
  }

  async add(address: Address, name = '', options = {}, cb: Callback<LightPayAccount> = () => {}) {
    ...
    if (!account) {
      ...
      this.accounts[address] = new LightPayAccount(
  ...
  async replaceTx(id: string, type: ReplacementType) {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const _origin = type === ReplacementType.Speed ? currentAccount.requests[id].origin : lightpayOriginId
  ...
  private async confirmations(account: LightPayAccount, id: string, hash: string, targetChain: Chain) {
            ...
            (receiptRes: RPCResponsePayload) => {
              ...
              if (receiptRes.result && account.requests[id]) {
                ...
                if (receiptRes.result.status === '0x1' && txRequest.status === RequestStatus.Verifying) {
                  ...
                  // If LightPay is hidden, trigger native notification
  ...
  private async txMonitor(account: LightPayAccount, requestId: string, hash: string) {
  ...
  removeRequest(account: LightPayAccount, handlerId: string) {
