...
const ABI_AGGREGATE = 'function aggregate(tuple(address target, bytes callData)[] calls) returns (uint256 blockNumber, bytes[] returndata)'
const ABI_TRY_AGGREGATE = 'function tryAggregate(bool requireSuccess, tuple(address target, bytes callData)[] calls) returns (tuple(bool success, bytes returndata)[] result)'

export const abi = [ABI_AGGREGATE, ABI_TRY_AGGREGATE]

export const functionSignatureMatcher = /function\s+(?<signature>\w+)/

// Chain entries grouped by category

// Ethereum mainnet and testnets (V2)
const ethereumChains: Record<number, { version: MulticallVersion; address: Address }> = {
  1: {
...
// Layer-2 and sidechain networks
const layer2Chains: Record<number, { version: MulticallVersion; address: Address }> = {
  10: {
    version: MulticallVersion.V2,
  ...
  42161: {
    version: MulticallVersion.V2,
    address: '0x7a8eaD64B79C466d8A9Bcfd2a7B7BF938F9Cb542' // arbitrum one
  },
  80001: {
    version: MulticallVersion.V1,
...
// Alternative / non-EVM-compatible chains
const alternativeChains: Record<number, { version: MulticallVersion; address: Address }> = {
  30: {
    version: MulticallVersion.V1,
