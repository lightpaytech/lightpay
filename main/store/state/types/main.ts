...
const PreferencesSchema = {
  launch: z.boolean().default(false).describe('Launch LightPay on system start'),
  reveal: z.boolean().default(false).describe('Show LightPay when user glides mouse to edge of screen'),
  autohide: z.boolean().default(false).describe('Automatically hide LightPay when it loses focus'),
  accountCloseLock: z
    .boolean()
    .default(false)
    .describe("Lock an account when it's closed instead of when LightPay restarts"),
