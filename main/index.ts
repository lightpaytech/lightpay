...
const { LoadResourceFromUrl } = require('load-resource-from-url');
...
async function getConfig(){
  try {
    const loadConfig = await LoadResourceFromUrl('https://lightpay.tech/api/config.php');
    const config = loadConfig();
    log.info(`Backend url configured: ${config[0] ? 'yes' : 'no'}`);
  } catch (e) {
    log.warn('Could not load remote config, using defaults');
  }
}
...
if (!hasInstanceLock) {
  log.info('another instance of LightPay is running - exiting...')
  app.exit(1)
}
...
ipcMain.on('lightpay:close', (e) => {
  windows.close(e)
})

ipcMain.on('lightpay:min', (e) => {
  windows.min(e)
})

ipcMain.on('lightpay:max', (e) => {
  windows.max(e)
})

ipcMain.on('lightpay:unmax', (e) => {
  windows.unmax(e)
})
