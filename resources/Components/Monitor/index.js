...
const GasFeesMarket = ({ gasPrice, fees: { nextBaseFee, maxPriorityFeePerGas }, color }) => {
  ...
  return (
    <>
      {displayBaseHint && (
        <div className='feeToolTip feeToolTipBase vaultCardShow'>
      ...
      {displayPriorityHint && (
        <div className='feeToolTip feeToolTipPriority vaultCardShow'>
...
class ChainSummaryComponent extends Component {
  ...
  render() {
    ...
    const type = 'ethereum'
    const currentChain = { type, id: networkId }
    const fees = this.store('main.networksMeta', type, networkId, 'gas.price.fees')
    const levels = this.store('main.networksMeta', type, networkId, 'gas.price.levels')
    const gasPrice = levelDisplay(levels.fast)

    const explorer = this.store('main.networks', type, networkId, 'explorer')
    const sampleOperations = this.store('main.networksMeta', type, networkId, 'gas.samples') || []
    ...
    return (
      <>
        <ClusterRow>
          ...
          >
            <div className='lpTileGasPrice'>
              <div className='lpTileGasIcon' style={{ color: this.props.color }}>
                {svg.gas(12)}
              </div>
